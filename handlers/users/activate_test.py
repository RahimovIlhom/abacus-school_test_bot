...
from keyboards.inline import make_all_tests_inlines, all_test_callback_data, activate_test_buttons
from loader import dp, db
from states import ActiveTestStates


@dp.message_handler(IsPrivate(), text="🔓 Testni faollashtirish", user_id=ADMINS)
async def show_tests(msg: types.Message, state: FSMContext):
    await state.update_data({'access': False})
    await msg.answer("Tanlang:", reply_markup=free_premium_buttons)
    await ActiveTestStates.free_premium.set()


@dp.message_handler(text=("🆓 Bepul testlar", "💰 Pullik testlar"), state=ActiveTestStates.free_premium)
async def choose_type(msg: types.Message, state: FSMContext):
    ...
    await ActiveTestStates.next()


@dp.message_handler(text="⬅️ Orqaga", state=ActiveTestStates.free_premium)
...
@dp.callback_query_handler(all_test_callback_data.filter(), state=ActiveTestStates.tests)
async def click_test(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    class_num = db.select_class(data[8])[1]
    science = db.select_science(data[9])[1]
    await state.update_data({
        'class_num': class_num,
        'science': science
    })
    info = f"ℹ️ Test haqida ma'lumot.\n\n" \
           f"🔢 Test sinfi: {class_num}-sinf\n\n" \
           f"📗 Test fani: {science} fani\n\n" \
           f"📊 Test holati: aktiv emas 🚫\n\n" \
           f"🔢 Test soni: {data[3]}\n\n" \
           f"📝 Javoblar: {data[4]}\n\n"
    if data[5]:
    ...
    try:
        await call.message.answer_photo(data[2], caption=f"{info}", reply_markup=activate_test_buttons)
    except BadRequest:
        await call.message.answer_document(data[2], caption=f"{info}", reply_markup=activate_test_buttons)
    await ActiveTestStates.next()


# ['id', 'permission', 'progress', 'start', 'quantity', 'score', 'certificate', 'check_image', 'test_id', 'user_id', 'stop']
@dp.callback_query_handler(text="statistics", state=ActiveTestStates.test)
...
@dp.callback_query_handler(text="activate", state=ActiveTestStates.test)
async def activate_test(call: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        test_id = data.get('test_id')
    db.update_access_test(test_id)
    await call.message.answer("Admin panel", reply_markup=admin_panel_buttons)
    await call.answer("Test aktivlashtirildi!", show_alert=True)
    await call.message.delete()
    await state.reset_data()
    await state.finish()


@dp.callback_query_handler(text="back_to_no_actives", state=ActiveTestStates.test)
async def activate_test(call: types.CallbackQuery, state: FSMContext):
    await call.message.delete()
    async with state.proxy() as data:
        is_active = data.get('access')
    ...
    await ActiveTestStates.tests.set()


@dp.callback_query_handler(text="back_to_admin_panel", state=ActiveTestStates.test)
async def activate_test(call: types.CallbackQuery, state: FSMContext):
    await call.message.delete()
    await state.reset_data()
    await call.message.answer("Admin panel", reply_markup=admin_panel_buttons)
...
@dp.message_handler(state=ActiveTestStates, content_types=ContentType.ANY)
